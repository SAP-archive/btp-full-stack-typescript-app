...
export = (srv: Service) => {
    ...
    srv.before("CREATE", "Lectures", async (req) => {
        ...
        if (lectures.length > 0) return req.error(400, "SELECTED_ROOM_NOT_AVAILABLE");
    });
    ...
    srv.on("createEvent", async (req) => {
        ...
        const overlappingLectures = (await srv
            .read(Lectures)
            .where({
    ...
    srv.after("READ", "Lectures", (each) => {
        ...
        if (lecture.starttime && lecture.endtime) {
            ...
            lecture.allday =
                ...
                new Date(lecture.starttime).toISOString() == end.toISOString();
        }
    });
